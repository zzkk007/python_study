...
从技术上讲：python3.0中 print 不是一个保留字，也不是一个语句，而是一个内置函数调用；由于它几乎总是作为一条表达式语句运行，
通常看作是一条语句类型。
...
在python3.0中，file关键字允许一个单个的print调用将其文件发送给一个文件的write方法，而不用
真正的重设sys.stdout。
...
代码块分隔符：
	...
	1、1python会自动以行缩进检测块的边界，就是程序代码左侧的空白空间。缩进至右侧相同的距离的所有语句属于同一块代码。
...
reload基础：
	...
	reload会在适当的地方修改模块对象，reload并不会删除并重建模块对象。因此，程序中任何引起该模块对象的地方，自动回
受到reload的影响。
...
"------------------------------------------------------------------------------------------------------"
	...
	除了模块名之外，导入也可以指定目录路径。python代码目录就称为包，因此，这类导入就称为包导入。
事实上，包导入时把计算机上的目录变成另一个Python命名空间，而属性则对应目录中的包含的子目录和模块文件。
...
相对导入基础知识：
	from语句现在可以使用前面的点号(".")来指定，它们需要位于同一包中的模块(所谓的包的相对导入)，而不是位于模块
导入搜索路径某处的模块（叫做决定导入）。
...
本章为第五部分结尾：数据隐藏、__future__模块、__name__变量、sys.path修改、列表工具、通过名称字符串来运行模块、
过渡式重载等。
...
本章小结：
	1、python的oop的重要意义是什么？
		OOP就是代码重用：分解代码、最小化代码的冗余以及对现有代码进行定制来编写代码，
		而不是实时修改代码，或从头开始。
...
第一个例子:
	...
	以OOP的观点看，x是FirstClass对象，y也是。如下下图：
...
步骤1 ：创建实例
	...
	实例对象属性是通过类方法函数中的self属性赋值来创建。在__init__构造函数方法中将它们赋给self。
